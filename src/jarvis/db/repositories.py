...
class ChatRepository:
    ...
    async def save_message(
        ...
        db = get_db()
        result = await run_db(lambda: db.table("chat_history").insert({
            "user_id": user_id,
            "role": role,
            "content": content,
    ...
    async def get_recent_messages(
        ...
        db = get_db()
        result = await run_db(lambda: db.table("chat_history")
            .select("*")
            .eq("user_id", user_id)
...
class MemoryRepository:
    ...
    async def save_fact(
        ...
        db = get_db()
        result = await run_db(lambda: db.table("memory_facts").insert({
            "user_id": user_id,
            "fact": fact,
            "category": category,
            "embedding": embedding,
    ...
    async def search_facts(
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("match_memory_facts", {
    ...
    async def get_all_facts(user_id: str) -> list[dict]:
        db = get_db()
        result = await run_db(lambda: db.table("memory_facts")
...
class RAGRepository:
    ...
    async def save_document(
        ...
        db = get_db()
        result = await run_db(lambda: db.table("rag_documents").insert({
            "user_id": user_id,
            "title": title,
            "content": content,
            "embedding": embedding,
    ...
    async def search_documents(
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("match_rag_documents", {
            "query_embedding": query_embedding,
            "match_user_id": user_id,
            "match_threshold": threshold,
            "match_count": limit
    ...
    async def delete_document(document_id: str) -> bool:
        db = get_db()
        result = await run_db(lambda: db.table("rag_documents")
            .delete()
            .eq("id", document_id)
            .execute())
...
class UserPreferencesRepository:
    ...
    async def get_or_create(user_id: str) -> dict:
        db = get_db()
        result = await run_db(lambda: db.table("user_preferences")
            .select("*")
            .eq("user_id", user_id)
            .execute())
        ...
        new_prefs = await run_db(lambda: db.table("user_preferences").insert({
            "user_id": user_id
        }).execute())
...
class LLMLogsRepository:
    ...
    async def get_costs_by_period(
        ...
        result = await run_db(lambda: db.rpc("get_llm_costs", params).execute())
        return result.data if result.data else []
    ...
    async def get_daily_stats(days: int = 30) -> list[dict]:
        ...
        db = get_db()
        result = await run_db(lambda: db.from_("llm_stats_daily")
            .select("*")
...
class TaskRepository:
    ...
    async def enqueue(
        ...
        result = await run_db(lambda: db.table("task_queue").insert(data).execute())
        return result.data[0] if result.data else None
    ...
    async def claim_next(worker_id: str) -> Optional[dict]:
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("claim_next_task", {"p_worker_id": worker_id}).execute())
        # RPC returns [None] or a dict with id=None when no task available
        if not result.data:
            return None
        task = result.data if isinstance(result.data, dict) else result.data[0] if isinstance(result.data, list) and result.data else None
    ...
    async def start_task(task_id: str) -> dict:
        ...
        db = get_db()
        result = await run_db(lambda: db.table("task_queue")
            .update({
                "status": "running",
    ...
    async def complete_task(task_id: str, result_data: dict = None) -> dict:
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("complete_task", {
            "p_task_id": task_id,
    ...
    async def fail_task(task_id: str, error: str) -> dict:
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("fail_task", {
            "p_task_id": task_id,
            "p_error": error
        }).execute())
        return result.data if result.data else None
    ...
    async def cleanup_stale_tasks(timeout_minutes: int = 30) -> int:
        ...
        db = get_db()
        result = await run_db(lambda: db.rpc("cleanup_stale_tasks", {
