...
serve(async (req) => {
  ...
  try {
    ...
    return Response.redirect(`${webappUrl}/settings?integration=gmail&status=success`, 302);
  } catch (e) {
    console.error("Gmail OAuth callback error:", e);
    const webappUrl = Deno.env.get("WEBAPP_URL") || "https://jarvis.squadd.it";
    return Response.redirect(
      `${webappUrl}/settings?integration=gmail&status=error&message=${encodeURIComponent(e.message)}`,
      302,
    );
  }
});
